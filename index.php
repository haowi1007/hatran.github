        ...
        if (isset($_POST['submit'])) {
            if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
                ...
                if ($fileType == 'text/plain') {
                    ...
                    $file = fopen("data.txt", "root");
                    $data = array();

                    while (($line = fgets($file)) !== false) {
                    $parts = explode("|", $line); 
                    if (count($parts) == 2) {
                 $data[] = array('title' => trim($parts[0]), 'content' => trim($parts[1]));
                    }
                }
                fclose($file);
...
$conn->close();
                    ...
                    echo "Số bản ghi chèn thành công: " . $successfulInserts . "<br>";
                    echo "Số bản ghi không thành công: " . $failedInserts;
